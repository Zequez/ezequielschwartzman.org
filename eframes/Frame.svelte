<script lang="ts">
  ...
  function handleMouseDown(from: 'topbar' | 'frame', ev: MouseEvent) {
    if (ev.button === 0) {
      if (from === 'topbar' || (from === 'frame' && ev.shiftKey)) {
        ev.stopPropagation()
        ev.preventDefault()
        moving = { x: p.x, y: p.y, z: FS.topZ() + 1, didMove: false }
      }
    }
  }
  ...
  function handleMouseMove(ev: MouseEvent) {
    if (moving) {
      moving = {
        x: moving.x + ev.movementX / CS.zoom,
        y: moving.y + ev.movementY / CS.zoom,
        z: moving.z,
        didMove: true,
      }
  ...
  let xy = $derived(moving || justMoved || p)
  ...
  let zIndex = $derived(
    FS.hoveringId === id ? 9999 + 1 : p.layer === 'bg' ? 18 : 20 + xy.z,
  )
...
{#snippet handlingBar()}
  {#if isDevMode && RS.editMode}
    ...
    >
      ...
      <div
        onmousedown={handleMouseDown.bind(null, 'topbar')}
        role="presentation"
...
{#if !p.draft || isDevMode}
  {#if p.layer === 'fg'}
    ...
    <div
      ...
      onmouseover={() => (!moving ? FS.cmd.hovering(id) : null)}
      onmousedown={handleMouseDown.bind(null, 'frame')}
      onclick={handleClick}
    >
      {@render handlingBar()}
      {#if !p.hidden}
